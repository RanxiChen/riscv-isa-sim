...
{
public:
  AmeIntAccRef(MatrixReg& reg, reg_t row, reg_t col, ameUnit_t& AMU)
    : reg(reg), row_index(row), col_index(col), AMU(AMU) {}

  void operator+=(const AmeDotExpr<AType, BType>& dot)
  {
    static_assert(std::is_integral_v<CType>);
    static_assert(std::is_signed_v<CType>);
    static_assert(std::is_integral_v<AType>);
    static_assert(std::is_integral_v<BType>);

    __int128 value = static_cast<__int128>(
        reg.elt<CType>(row_index, col_index));

    for (reg_t k = 0; k < dot.lhs.size(); ++k) {
      const __int128 lhs = static_cast<__int128>(dot.lhs.elt(k));
      const __int128 rhs = static_cast<__int128>(dot.rhs.elt(k));
      value += lhs * rhs;
    }

    const __int128 min_value = static_cast<__int128>(
        std::numeric_limits<CType>::min());
    const __int128 max_value = static_cast<__int128>(
        std::numeric_limits<CType>::max());

    CType result;
    if (AMU.xmsaten_enabled()) {
      if (value > max_value) {
        result = std::numeric_limits<CType>::max();
        AMU.xmsat->write_raw(
            (AMU.get_xmsat() | 1) & ameUnit_t::XMSAT_MASK);
      } else if (value < min_value) {
        result = std::numeric_limits<CType>::min();
        AMU.xmsat->write_raw(
            (AMU.get_xmsat() | 1) & ameUnit_t::XMSAT_MASK);
      } else {
        result = static_cast<CType>(value);
      }
    } else {
      using UType = std::make_unsigned_t<CType>;
      const UType wrapped = static_cast<UType>(value);
      result = std::bit_cast<CType>(wrapped);
    }

    reg.elt<CType>(row_index, col_index) = result;
  }
...
// --- integer type/bits mappings ---
#define AME_MMACC_TYPE_I8  int8_t
#define AME_MMACC_TYPE_U8  uint8_t
#define AME_MMACC_TYPE_I32 int32_t

#define AME_MMACC_BITS_I8   8
#define AME_MMACC_BITS_U8   8
#define AME_MMACC_BITS_I32 32
...
// --- integer core execution macro (no require_matrix_ms / AME_END) ---
// Parameters: CT/AT/BT are type tokens (I32, I8, U8)
// FEATURE is the xmisa bit enum name (e.g. XMISA_BIT_MMI8I32)
// BODY is the instruction body block
#define AME_MMACC_IMPL(CT, AT, BT, FEATURE, BODY)   do {     auto& AMU = p->AMU;     const reg_t md_field = insn.m_md();     const reg_t ms1 = insn.m_ms1();     const reg_t ms2 = insn.m_ms2();     ame_require(md_field >= 4 && md_field < 8, insn);     ame_require(ms1 < 4, insn);     ame_require(ms2 < 4, insn);     ame_require(AMU.supports_xmisa_feature(ameUnit_t::xmisa_bit(ameUnit_t::FEATURE)), insn);     ame_require(AMU.get_elen() >= AME_MMACC_BITS_##CT, insn);     const reg_t M = AMU.mtilem->read();     const reg_t N = AMU.mtilen->read();     const reg_t K = AMU.mtilek->read();     ame_require(M <= AMU.get_rownum(), insn);     ame_require(N <= AMU.get_rownum(), insn);     ame_require(K <= AMU.get_trlen() / AME_MMACC_BITS_##AT, insn);     MatrixReg& Areg = AMU.tile_regs[ms1];     MatrixReg& Breg = AMU.tile_regs[ms2];     MatrixReg& Creg = AMU.acc_regs[md_field - 4];     AmeMatrixView<AME_MMACC_TYPE_##AT> A(Areg, K);     AmeMatrixView<AME_MMACC_TYPE_##BT> B(Breg, K);     for (reg_t i = 0; i < M; ++i) {       for (reg_t j = 0; j < N; ++j) {         AmeIntAccRef<AME_MMACC_TYPE_##CT, AME_MMACC_TYPE_##AT, AME_MMACC_TYPE_##BT> Cij(Creg, i, j, AMU);         BODY;       }     }     const AME_MMACC_TYPE_##CT zero = {0};     const reg_t acc_rows = Creg.get_rownum();     const reg_t acc_cols = Creg.get_row_bytes() / sizeof(AME_MMACC_TYPE_##CT);     for (reg_t i = 0; i < acc_rows; ++i) {       for (reg_t j = 0; j < acc_cols; ++j) {         if (i >= M || j >= N)           Creg.elt<AME_MMACC_TYPE_##CT>(i, j) = zero;       }     }   } while (0);

// --- outer wrapper: require_matrix_ms + core + AME_END ---
#define AME_MMACC_INT(CT, AT, BT, FEATURE, BODY)   do {     require_matrix_ms;     AME_MMACC_IMPL(CT, AT, BT, FEATURE, BODY);     AME_END;   } while (0);

// --- per-instruction macros ---
#define AME_MMACC_I32_I8_I8(BODY)   AME_MMACC_INT(I32, I8, I8, XMISA_BIT_MMI8I32, BODY)
#define AME_MMACC_I32_U8_U8(BODY)   AME_MMACC_INT(I32, U8, U8, XMISA_BIT_MMI8I32, BODY)
#define AME_MMACC_I32_U8_I8(BODY)   AME_MMACC_INT(I32, U8, I8, XMISA_BIT_MMI8I32, BODY)
#define AME_MMACC_I32_I8_U8(BODY)   AME_MMACC_INT(I32, I8, U8, XMISA_BIT_MMI8I32, BODY)

// --- integer execute functions for internal tests ---
